...
def main():
    ...
    # ===============================
    # 1) PARCEL / LANDUSE checks
    # ===============================
    assert_zero(
        cur,
        "SELECT COUNT(*) FROM raw.parcel_raw WHERE geom IS NULL OR NOT ST_IsValid(geom);",
        "Invalid parcel geometries"
    )

    assert_zero(
        cur,
        "SELECT COUNT(*) FROM raw.parcel_raw WHERE area_ha IS NULL OR area_ha <= 0;",
        "Non-positive parcel area"
    )

    assert_positive(
        cur,
        "SELECT COUNT(*) FROM raw.parcel_raw;",
        "Parcel records loaded"
    )

    assert_positive(
        cur,
        "SELECT COUNT(DISTINCT year) FROM raw.parcel_raw;",
        "Parcel years loaded"
    )
    ...
    assert_positive(
        cur,
        "SELECT COUNT(*) FROM dm.f_parcel_usage;",
        "DM parcel usage"
    )
