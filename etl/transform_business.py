...
def main():
    ...
    cur.execute("""
        ...
        total AS (
            SELECT year, SUM(area_ha) AS total_area
            FROM parcel_current
            GROUP BY year
        )
        SELECT
            ...
            p.category,
            COUNT(*)::integer,
            ROUND(SUM(p.area_ha)::numeric, 2)::double precision,
            ROUND((SUM(p.area_ha) / NULLIF(t.total_area, 0) * 100)::numeric, 2)::double precision
        FROM parcel_current p
        JOIN total t ON p.year = t.year
        GROUP BY p.year, p.category, t.total_area
        ORDER BY p.year, SUM(p.area_ha) DESC;
    """)
    ...
    cur.execute("""
        ...
        land AS (
            SELECT
                year,
                SUM(area_ha)::numeric AS total_area_ha,
                SUM(CASE WHEN category IN ('farmland', 'agricultural', 'agriculture') THEN area_ha ELSE 0 END)::numeric AS agriculture_area_ha,
                SUM(CASE WHEN category = 'forest' THEN area_ha ELSE 0 END)::numeric AS forest_area_ha,
                SUM(CASE WHEN category = 'residential' THEN area_ha ELSE 0 END)::numeric AS residential_area_ha,
                SUM(CASE WHEN category = 'industrial' THEN area_ha ELSE 0 END)::numeric AS industrial_area_ha
            FROM dv.sat_parcel_thematic
            GROUP BY year
        ),
        land_2013 AS (SELECT * FROM land WHERE year = 2013),
        land_2023 AS (SELECT * FROM land WHERE year = 2023),
        roads AS (
            SELECT
        ...
        UNION ALL
        SELECT 'Agricultural land change 2013-2023',
               ROUND(land_2013.agriculture_area_ha, 2)::double precision,
               ROUND(land_2023.agriculture_area_ha, 2)::double precision,
               ROUND(((land_2023.agriculture_area_ha - land_2013.agriculture_area_ha) / NULLIF(land_2013.agriculture_area_ha, 0) * 100), 2)::double precision
        FROM land_2013, land_2023

        UNION ALL
        SELECT 'Forest land change 2013-2023',
               ROUND(land_2013.forest_area_ha, 2)::double precision,
               ROUND(land_2023.forest_area_ha, 2)::double precision,
               ROUND(((land_2023.forest_area_ha - land_2013.forest_area_ha) / NULLIF(land_2013.forest_area_ha, 0) * 100), 2)::double precision
        FROM land_2013, land_2023

        UNION ALL
        SELECT 'Residential land change 2013-2023',
               ROUND(land_2013.residential_area_ha, 2)::double precision,
               ROUND(land_2023.residential_area_ha, 2)::double precision,
               ROUND(((land_2023.residential_area_ha - land_2013.residential_area_ha) / NULLIF(land_2013.residential_area_ha, 0) * 100), 2)::double precision
        FROM land_2013, land_2023

        UNION ALL
        SELECT 'Industrial land change 2013-2023',
               ROUND(land_2013.industrial_area_ha, 2)::double precision,
               ROUND(land_2023.industrial_area_ha, 2)::double precision,
               ROUND(((land_2023.industrial_area_ha - land_2013.industrial_area_ha) / NULLIF(land_2013.industrial_area_ha, 0) * 100), 2)::double precision
        FROM land_2013, land_2023
