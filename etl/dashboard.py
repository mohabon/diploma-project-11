...
def load_data():
    ...
    data["chapter3_results"] = read_sql_safe("""
        SELECT indicator, value_start, value_end, change_percent
        FROM dm.f_chapter3_results
        ORDER BY indicator
    """, conn)
    ...
    data["dv_counts"] = read_sql_safe("""
        SELECT 'hub_parcel' AS name, COUNT(*) AS rows FROM dv.hub_parcel
        ...
        UNION ALL SELECT 'link_parcel_settlement', COUNT(*) FROM dv.link_parcel_settlement
        UNION ALL SELECT 'link_road_settlement', COUNT(*) FROM dv.link_road_settlement
        UNION ALL SELECT 'sat_parcel_thematic', COUNT(*) FROM dv.sat_parcel_thematic
        UNION ALL SELECT 'sat_parcel_geom', COUNT(*) FROM dv.sat_parcel_geom
    ...
    data["raw_counts"] = read_sql_safe("""
        SELECT 'raw.parcel_raw' AS table_name, COUNT(*) AS rows FROM raw.parcel_raw
    ...
    data["dq"] = read_sql_safe("""
        SELECT
            (SELECT COUNT(*) FROM raw.parcel_raw WHERE geom IS NOT NULL AND NOT ST_IsValid(geom)) AS invalid_parcels,
...
landuse_ru = {
    "residential": "Застроенные территории",
    "farmland": "Сельскохозяйственные угодья",
    "agricultural": "Сельскохозяйственные угодья",
    "agriculture": "Сельскохозяйственные угодья",
    "forest": "Леса",
    "industrial": "Промышленность/инфраструктура",
    "жилая": "Застроенные территории",
    "сельхоз": "Сельскохозяйственные угодья",
    "сельскохозяйственные земли": "Сельскохозяйственные угодья",
    "лес": "Леса",
    "леса": "Леса",
    "промышленность": "Промышленность/инфраструктура",
}
...
def indicator_ru(indicator):
    mapping = {
        "Agricultural land change 2013-2023": "Сельскохозяйственные угодья, 2013–2023",
        "Forest land change 2013-2023": "Леса, 2013–2023",
        "Residential land change 2013-2023": "Застроенные территории, 2013–2023",
        "Industrial land change 2013-2023": "Промышленность/инфраструктура, 2013–2023",
        "Population growth 2013-2023": "Численность населения, 2013–2023",
        "Employment rate change 2013-2023": "Уровень занятости, 2013–2023",
        "Average income index growth 2013-2023": "Индекс среднего дохода, 2013–2023",
        "PM2.5 growth 2018-2023": "PM2.5, 2018–2023",
        "NO2 growth 2018-2023": "NO₂, 2018–2023",
...
with tabs[0]:
    ...
    latest_landuse_year = int(data["landuse"]["year"].max()) if "landuse" in data and not data["landuse"].empty else None
    land_latest = data["landuse"][data["landuse"]["year"] == latest_landuse_year] if latest_landuse_year else pd.DataFrame()

    total_land = int(land_latest["objects_count"].sum()) if not land_latest.empty else 0
    total_area = round(land_latest["total_area_ha"].sum(), 2) if not land_latest.empty else 0
    ...
    mart_count = 6 if "chapter3_results" in data and not data["chapter3_results"].empty else 5
    ...
    h.metric("Годы анализа земель", "2013 / 2023")

    st.info(
        "Рабочий прототип построен по цепочке: Raw Layer → Data Vault → Data Marts → Streamlit Dashboard. "
        "Слой землепользования хранится в raw.parcel_raw и анализируется через dm.f_parcel_usage."
    )


# ==================== Tab 1: Sources and ETL ====================
with tabs[1]:
    ...
    sources = pd.DataFrame([
        ["landuse_2013.geojson", "raw.parcel_raw", "Землепользование за 2013 год"],
        ["landuse_2023.geojson", "raw.parcel_raw", "Землепользование за 2023 год"],
...
with tabs[3]:
    ...
    dv_catalog = pd.DataFrame([
        ["hub_parcel", "Hub", "Бизнес-ключи объектов землепользования"],
        ...
        ["link_parcel_settlement", "Link", "Пространственная связь землепользования с территорией"],
        ["link_road_settlement", "Link", "Пространственная связь дорожной сети с территорией"],
        ["sat_parcel_thematic", "Satellite", "Атрибуты объектов землепользования по годам"],
        ["sat_parcel_geom", "Satellite", "Геометрия объектов землепользования по годам"],
    ...
    st.dataframe(dv_catalog, use_container_width=True, height=600, hide_index=True)
...
with tabs[4]:
    ...
    mart_catalog = pd.DataFrame([
        ["dm.f_parcel_usage", "Землепользование", "Год + тип землепользования", "Уровень всей мухафазы", "Землепользование, карта"],
        ...
        ["dm.f_chapter3_results", "Итоговые результаты", "Показатель", "Сводные значения", "Результаты и рекомендации"],
    ...
    st.dataframe(mart_catalog, use_container_width=True, height=360, hide_index=True)
...
with tabs[5]:
    ...
    if "roads" in data and not data["roads"].empty:
        ...
        st.dataframe(roads, use_container_width=True, hide_index=True)
...
with tabs[6]:
    ...
    if "landuse" in data and not data["landuse"].empty:
        ...
        lu["category_ru"] = lu["category"].replace(landuse_ru)

        available_years = sorted(lu["year"].unique())
        selected_landuse_year_tab = st.selectbox(
            "Год анализа землепользования",
            options=available_years,
            index=len(available_years) - 1,
            key="landuse_tab_year",
        )

        lu_year = lu[lu["year"] == selected_landuse_year_tab].copy()
        lu_year = lu_year.rename(columns={
            "category_ru": "Тип землепользования",
            "objects_count": "Количество объектов",
            "total_area_ha": "Общая площадь, га",
            "share_percent": "Доля, %",
        })

        st.subheader(f"Структура землепользования за {selected_landuse_year_tab} год")
        st.dataframe(
            lu_year[["Тип землепользования", "Количество объектов", "Общая площадь, га", "Доля, %"]],
        ...
        with col1:
            ...
            st.bar_chart(lu_year.set_index("Тип землепользования")["Доля, %"])
        with col2:
            st.markdown("**Площадь по типам землепользования**")
            st.bar_chart(lu_year.set_index("Тип землепользования")["Общая площадь, га"])

        st.subheader("Динамика землепользования 2013–2023")

        pivot = lu.copy()
        pivot["category_ru"] = pivot["category"].replace(landuse_ru)

        area_pivot = pivot.pivot_table(
            index="category_ru",
            columns="year",
...
with tabs[7]:
    ...
    if "eco" in data and not data["eco"].empty:
        ...
        st.dataframe(eco_ru, use_container_width=True, hide_index=True)
...
with tabs[8]:
    ...
    if "population" in data and not data["population"].empty:
        ...
        st.dataframe(pop_ru, use_container_width=True, hide_index=True)
        ...
        st.subheader("Сводные показатели по мухафазе")
...
# ==================== Tab 9: Results and recommendations ====================
with tabs[9]:
    st.header("📌 Результаты и рекомендации")

    st.markdown(
        "Раздел обобщает ключевые аналитические выводы прототипа "
        "и формулирует планировочные рекомендации для мухафазы Тартус."
    )

    if "chapter3_results" in data and not data["chapter3_results"].empty:
        res = data["chapter3_results"].copy()
        res["Показатель"] = res["indicator"].apply(indicator_ru)
        res = res.rename(columns={
            "value_start": "Начальное значение",
            "value_end": "Конечное значение",
            "change_percent": "Изменение, %",
        })

        st.subheader("Итоговые показатели апробации")
        st.dataframe(
            res[["Показатель", "Начальное значение", "Конечное значение", "Изменение, %"]],
            use_container_width=True,
            hide_index=True,
        )
...
with tabs[10]:
    ...
    with col1:
        show_population = st.checkbox("Население и экономика", value=True)
    ...
    selected_population_year = None

    if show_population and "population" in data and not data["population"].empty:
        available_pop_years = sorted(data["population"]["year"].unique())
        preferred_pop_years = [2013, 2018, 2023]
        pop_year_options = [y for y in preferred_pop_years if y in available_pop_years]

        if not pop_year_options:
            pop_year_options = available_pop_years

        selected_population_year = st.selectbox(
            "Год анализа населения и экономики",
            options=pop_year_options,
            index=len(pop_year_options) - 1,
        )

    selected_landuse_year = None

    if show_landuse and "landuse_map" in data and not data["landuse_map"].empty:
        landuse_year_options = sorted(data["landuse_map"]["year"].unique())
        selected_landuse_year = st.selectbox(
            "Год анализа землепользования",
            options=landuse_year_options,
            index=len(landuse_year_options) - 1,
            key="landuse_map_year",
        )

    selected_eco_year = None

    if show_ecology and "eco" in data and not data["eco"].empty:
    ...
    if (
        ...
        and "population" in data
        and not data["population"].empty
        and selected_population_year is not None
    ):
        selected_pop = data["population"][data["population"]["year"] == selected_population_year].copy()
        selected_pop["district_key"] = selected_pop["district"].apply(get_district_key)
        ...
        pop_dict = {
            ...
            for _, row in selected_pop.iterrows()
        }
        ...
        population_group = folium.FeatureGroup(
            name=f"Население и экономика, {selected_population_year}",
            show=True,
        )
        ...
        for feature in districts_geojson["features"]:
            ...
            if info:
                ...
                tooltip = f"""
                <b>{ru_name}</b><br>
                Год данных: {selected_population_year}<br>
                ...
                tooltip = f"""
                <b>{ru_name}</b><br>
                Нет социально-экономических данных за {selected_population_year} год
                """
    ...
    if (
        ...
        and not data["landuse_map"].empty
        and selected_landuse_year is not None
    ):
        landuse_group = folium.FeatureGroup(
            name=f"Землепользование, {selected_landuse_year}",
            show=True,
        )

        landuse_map = data["landuse_map"][data["landuse_map"]["year"] == selected_landuse_year].copy()
        ...
        for _, row in landuse_map.iterrows():
            try:
                ...
                ru_type = landuse_ru.get(category, category)
                color = landuse_colors.get(category, "#808080")
                ...
                folium.GeoJson(
                    ...
                    tooltip=folium.Tooltip(
                        f"<b>{ru_type}</b><br>Год: {selected_landuse_year}<br>Площадь: {round(row['area_ha'], 2)} га",
                        sticky=True,
                    ),
    ...
    if (
        ...
        for feature in districts_geojson["features"]:
            ...
            if info:
                ...
                tooltip = f"""
                <b>{ru_name}</b><br>
                Год данных: {selected_eco_year}<br>
                PM2.5: {info["pm25"]} мкг/м³<br>
                NO₂: {info["no2"]} мкг/м³<br>
                Качество воды: {info["water_quality"]} индекс<br>
                NDVI: {info["ndvi"]}<br>
                ...
                tooltip = f"""
                <b>{ru_name}</b><br>
                Нет экологических данных за {selected_eco_year} год
                """
