...
# ==================== UPDATED LANDUSE LOADER ====================
def load_landuse(cur, geojson_path, year):
    with open(geojson_path, encoding="utf-8") as f:
        data = json.load(f)
    ...
    for feature in data["features"]:
        ...
        parcel_id = str(osm_id) if osm_id else f"{year}_{loaded}"

        geom = shape(geometry)
        geom_wkt = geom.wkt

        sql = """
        INSERT INTO raw.parcel_raw
        (parcel_id, year, category, name, area_ha, geom, load_ts, record_source)
        VALUES (%s, %s, %s, %s,
        ...
        exec_sql(
            ...
            [
                ...
                geom_wkt,
                geom_wkt,
                f"landuse_{year}_geojson",
            ],
        )

        loaded += 1

    print(f"Loaded landuse {year}: {loaded}, skipped non-polygon features: {skipped}")
...
def main():
    ...
    cur.execute("DELETE FROM raw.parcel_raw")
    ...
    # ======= IMPORTANT: load two years =======
    load_landuse(cur, "/app/sample_data/landuse_2013.geojson", 2013)
    load_landuse(cur, "/app/sample_data/landuse_2023.geojson", 2023)
    ...
    print("Loaded RAW data: settlements + statistics + parcel/landuse + roads")
